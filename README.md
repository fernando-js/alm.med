...
5. A conferência de medicamentos em `/medicamentos` consulta primeiro a tabela local `medication_guidance_rules`, sem chamar IA. Depois consulta `medication_guidance_cache`, que guarda resultados anteriores por medicamento + contexto clínico normalizado. A OpenAI só é usada como fallback para medicamentos sem regra e sem cache. Se quiser restringir o acesso, preencha `app.medication_access_code`; se ficar vazio, a ferramenta não exigirá código.
...
11. A tabela inicial de medicamentos é semeada por `api/data/medication_guidance_rules.php` e atualizada automaticamente no banco quando o endpoint é usado. Resultados gerados por IA ou já avaliados também ficam em `medication_guidance_cache` para evitar nova chamada em pesquisa semelhante. Revise e adapte essa base ao protocolo institucional antes de usar como orientação operacional.
12. A conferência de medicamentos e a minuta de APA são apoio à triagem: não substituem avaliação médica, protocolo institucional ou orientação individualizada do anestesiologista/cirurgião/médico prescritor.
13. Não versionar nem compartilhar `config.php`.
