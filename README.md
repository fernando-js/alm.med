...
5. A conferência de medicamentos em `/medicamentos` consulta primeiro a tabela local `medication_guidance_rules`, sem chamar IA. A OpenAI só é usada como fallback para medicamentos sem regra cadastrada. Se quiser restringir o acesso, preencha `app.medication_access_code`; se ficar vazio, a ferramenta não exigirá código.
...
11. A tabela inicial de medicamentos é semeada por `api/data/medication_guidance_rules.php` e atualizada automaticamente no banco quando o endpoint é usado. Revise e adapte essa base ao protocolo institucional antes de usar como orientação operacional.
